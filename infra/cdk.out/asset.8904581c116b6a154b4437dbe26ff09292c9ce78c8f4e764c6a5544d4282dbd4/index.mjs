...
async function listTickets() {
  const result = await ddb.send(new ScanCommand2({ TableName: TABLE }));
  const items = result.Items || [];
  return items.sort(
    (a5, b5) => new Date(b5.createdAt) - new Date(a5.createdAt)
  );
}
...
var handler = async (event) => {
  ...
  try {
    ...
    console.error("\u274C Error listing tickets:", err2);
    ...
    return createResponse(500, { error: "Failed to fetch tickets", details: err2.message });
  }
};
export {
  handler
