...
const [completedDrives, setCompletedDrives] = useState([]);
  ...
  min={new Date().toISOString().split("T")[0]}  // 👈 This line ensures only today or future dates can be selected
...
/>
  ...
  min={
    form.lastDateToApply
      ? new Date(new Date(form.lastDateToApply).getTime() + 86400000) // +1 day in ms
          .toISOString()
          .split("T")[0]
      : ""
  }
...
/>
        ...
        {activeTab === 'scheduled' && (
          <div className="scheduled-drives-section">
  ...
  onClick={async () => {
    try {
      ...
      if (hasError) {
        setPopup(errorMsg);
        setTimeout(() => setPopup(""), 4000); // shows popup for 4 seconds
        return;
      }
...
<p><strong>Unplaced:</strong> {
  d.registered?.filter((s) => s.status === "NOT PLACED").length || 0
        ...
        {activeTab === 'completed' && (
  <div className="completed-drives-section">
...
>
  ...
  <tbody>
    {companies.map((c) => (
      ...
      {popup && (
  <div className={`dashboard-popup ${popup.startsWith("✅") ? "success" : "error"}`}>
    {popup}
  </div>
)}
...
>
  ...
  <option value="PENDING">PENDING</option> {/* ✅ Corrected this line */}
